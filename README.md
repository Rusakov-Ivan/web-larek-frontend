...
Конструктор принимает исходные данные, инстант брокера событий.
...
Класс расширяет абстрактный класс `Component<T>` при помощи типа `TProductInBasket`\
и интерфейсом `IIndex`.
...
Конструктор использует параметры `container` типа `HTMLElement` и `action` типа\
`ICardActions`.\
Параметр `container` передается в родительский конструктор.\
При помощи функции `ensureElement` и `querySelector` происходит сохранение элеметов разметки.\
...
Конструктор принимает параметры `container` типа `HTMLElement` и `actions` типа `ICardActions`.\
Параметр `container` передается в родительский конструктор.\
При помощи функции `ensureElement` и `querySelector` происходит сохранение элеметов разметки.\
