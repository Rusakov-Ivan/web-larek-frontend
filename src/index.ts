...
import { CardProduct, ProductInBasket } from './components/view/CardProduct';
...
events.on('product:openModal', (product: IProduct) => {
	...
	modal.render({
		content: card.render({
			...
			price: product.price,
			state: product.price === null || appData.stateCard(product),
		}),
	});
});
...
events.on(
	...
	(data: { field: keyof Omit<IOrder, 'items' | 'total'>; value: string }) => {
		...
		addressForm.payment = appData.getPaymentMethod();
	}
);
